...
def payload():
    ...
    return {
        'service': 'Dosimetria Pré-Clínica',
        'researchLine': 'Linha de pesquisa 1',
        'equipmentType': 'SPECT',
        'equipmentModality': 'MICROSPECT_CT',
        'numberOfPatients': '5',
        'frequency': 'Mensal',
        'comments': comments,
    }
...
def test_invalid_equipment_modality_match(client_api_auth, user, payload):
    ...
    payload['equipmentModality'] = 'MICROPET_CT'
    ...
    assert resp.status_code == HTTPStatus.BAD_REQUEST
    assert 'A modalidade do equipamento deve ser MICROSPECT_CT se o tipo for SPECT.' in body['errors'][0]


def test_invalid_pet_equipment_modality_match(client_api_auth, user, payload):
    payload['equipmentType'] = 'PET'
    payload['equipmentModality'] = 'MICROSPECT_CT'
    ...
    assert resp.status_code == HTTPStatus.BAD_REQUEST
    assert 'A modalidade do equipamento deve ser MICROPET_CT se o tipo for PET.' in body['errors'][0]
